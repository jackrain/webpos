...
//创建一个校验器 name:名称 ,message:错误消息 ,fn:校验函数 ,env:校验环境 默认(0)前台后台都可以校验 1:仅在前台 2:仅在后台
...
ModuleValidator.prototype.Validate = function (v, element, context, callback) {
    ...
    if (this.Enviroment == ENV.OnlyBack && element != null) {
        ...
        if (typeQues.isFunction(this.ValidateHandler)) {
            ...
            r = this.ValidateFinal(n, context, callback);
        }
    }
    return r;
}
//校验完毕事件
ModuleValidator.prototype.ValidateFinal = function (n, context, callBack) {
...
Factory.CreateValidator('remote', messages.remote, Factory.Env.OnlyForm, function (value, element, param, context, callback) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            self.ValidateFinal(valid, context, callback);
        },
        error: function () {
            self.ValidateFinal(false, context, callback);
        }
    });
});