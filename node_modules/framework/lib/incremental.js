...
/*
 * 名称：同步增量数据
 * 参数(increpath): 增量包名称 例如: posbase_v927_plug.7z 注意：增量包默认会读取根目录下的dbupdate目录下的文件
 */
...
/*
 * 名称：1.解压增量包
 * 参数(zipname)：增量包名称 例如: posbase_v927_plug.7z 注意：增量包默认会读取根目录下的dbupdate目录下的文件
 * 参数(callback)：解压完毕后的回调函数 callback(basedb,updatedb) basedb:本地库连接路径 updatedb:增量包数据库连接路径
 */
function extractIncrementZip(zipname, callback) {
    ...
    zip.then(function () {
        LogUtil.Info(framework.InfoMessageStrings.extractIncrementSuccessString);
        LogUtil.Info(framework.InfoMessageStrings.startSyncIncrementString);
        framework.Call(callback, basedb, getIncrementZipDB(allFiles));
    })
    zip.catch(function () {
        LogUtil.Info(framework.InfoMessageStrings.extractIncrementErrorString);
    });
}

/*
 * 名称：2.生成增量数据同步
 * 参数(attachdb)：附加数据库，这里指的是 增量包数据库连接路径
 */
...
function sync(basedb, updatedb) {
    ...
    accessor.run(sql, function (err) {
        ...
        if (err) {
            ...
            LogUtil.Info(framework.InfoMessageStrings.syncDataIncrementSuccessString);
        }
    });
}
