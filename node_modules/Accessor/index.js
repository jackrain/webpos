...
//监听类型列表
SQLiteInnerAccessor.prototype.observerTypes = ["SELECT", "UPDATE", "DELETE", "INIT", "INSERT"];
//SQLite操作工具初始化
SQLiteInnerAccessor.prototype.init = function (tablename) {
    //初始化初始字段
    this.initPropertys(tablename);
    //初始化先关依赖引用
    this.initUsingRequires();
    //实例化数据库以及字段信息
    this.createdb();
    //初始化优先级队列
...
//初始化相关属性
SQLiteInnerAccessor.prototype.initPropertys = function (_tablename, autoTrans, options) {
    //表名
...
//初始化引入数据库配置
SQLiteInnerAccessor.prototype.initUsingRequires = function () {
    this.cfg = require(path.join(__dirname, "..", "..", "config", "configuration"));
    this.checkQuestion = framework.CheckQuestion;
    this.sqlBuilder = require('./lib/sql.builder.js');
}
//初始化SQLite数据库对象
SQLiteInnerAccessor.prototype.createdb = function (autoTrans) {
    //如果不是自动提交事务
    var self = this;
    if (autoTrans != true) {
        self.db = self.checkQuestion.noInObj(root, 'sharedb', function () { return new sqlite3.Database(self.getConnString()); })
    } else {
...
SQLiteInnerAccessor.prototype.insert = function (dataObject, callback) {
    ...
    self.query(sql, function (err, info) {
        self.transAsk();//询问事务计数器上升1
        self.dispatch("INSERT");
        process.nextTick(async.apply(callback, err, info));
    });
}
//修改
...
SQLiteInnerAccessor.prototype.select = function () {
    ...
    self.query(sql, function (err, dataset, fields) {
        self.transAsk();//询问事务计数器上升1
        self.dispatch("SELECT");
        process.nextTick(async.apply(callback, err, dataset, fields));
    });
}
//只读只进查询
SQLiteInnerAccessor.prototype.selectEach = function (options, callback, complete) {
    ...
    self.queryEach(sql, function (err, row) {
        ...
        self.transAsk();//询问事务计数器上升1
        self.dispatch("SELECT");
        process.nextTick(async.apply(complete, err, rows));
    });
}
//开始事务
SQLiteInnerAccessor.prototype.beginTrans = function (callback) {
    if (this.isTransCaculing) {
        throw new Error("事务没有结束,无法重新开启");
    }
    if (this.isBeginTrans) { return; }
    var self = this;
    self.transTotal = 0;
    self.isBeginTrans = true;//设置为已经开始事务
...
SQLiteInnerAccessor.prototype.commit = function (callback) {
    if (!this.isBeginTrans) { return; }
    var self = this;
    self.isTransCaculing = true;
    self.isBeginTrans = false;//设置为事务结束
    self.stopCargo();
    //添加提交到最后优先级队列
    self.addBottomLevel(function (innerCall) {
        self.exec('commit transaction', function () {
            self.isTransCaculing = false;
            innerCall();
            if (self.checkQuestion.isFunction(callback)) {
                callback();
            }
...
//事务结束
SQLiteInnerAccessor.prototype.transFinal = function () {
...
//事务结束监听
SQLiteInnerAccessor.prototype.transAsk = function () {
    var self = this;
...
SQLiteInnerAccessor.prototype.query = function (sql, callback) {
    if (!this.tryConnect()) {
        return callback(self.error("ERROR: 没有数据库连接,请检查是否配置连接路径"));
    }
    var self = this;
    var proc = process;
    self.db.parallelize(function () {
        self.db.all(sql, function (err, data) {
            if (err) {
                self.log("ERROR: 数据库语句执行错误, 原因： " + err + ", 语句： " + sql);
...
SQLiteInnerAccessor.prototype.exec = function (sql, callback, fields) {
    if (!this.tryConnect()) {
        return callback(self.error("ERROR: no database connection."));
    }
    var self = this;
    var db = self.db;
    var proc = process;
...
SQLiteInnerAccessor.prototype.dispatch = function (event, arg1, argN) {
    ...
    if (this.checkQuestion.isArray(handlers)) {
        async.parallel(handlers, function (error, results) {
            if (error) {
                self.log('执行 监听事件 ：' + event + ' 出现异常! 详情：' + error);
            }
        });
    }
}
...
SQLiteInnerAccessor.prototype.supportField = function (field) {
    ...
    if (!v) {
        this.log("Warning: 字段:[" + field + "] 在表中不存在，已忽略此字段的相关操作.");
    }
    return v;
}
...
SQLiteInnerAccessor.prototype.validateFields = function (log) {
    ...
    if (!v) {
        this.log("Warning: 找不到字段元数据,请检查是否有初始化，或者数据库表是否存在字段.");
    }
    return v;
}
...
SQLiteInnerAccessor.prototype.addAccessorHandle = function (method, argus) {
    ...
    if (this.isBeginTrans) {
        this.transTotal++;//计数参加事务的sql操作数量
    }
    if (self.isTransCaculing) {
...
//公布给模块外部的数据库操作类
function SQLiteAccessor(tablename, autoTrans, options) { init.apply(this, arguments); }

//开始事务
SQLiteAccessor.prototype.beginTrans = function (callback) {
    return innerAccessor.beginTrans(callback);
}
//回滚事务
