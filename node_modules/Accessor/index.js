...
//字段名称
SQLiteInnerAccessor.prototype.idAttribute = null;
//主键是否自增
SQLiteInnerAccessor.prototype.autoIncrement = false;
...
SQLiteInnerAccessor.prototype.initPropertys = function (_tablename, trans, options) {
    ...
    options = options || {}
    this.idAttribute = framework.Util.Undef(options.idAttribute, null);
    this.autoIncrement = framework.Util.Undef(options.autoIncrement, false);
}
//初始化引入数据库配置
...
//切换表名
SQLiteInnerAccessor.prototype.changeTable = function (newtable, id, autoIncrement) {
    if (framework.String.Equals(newtable, this.tablename, true)) {
        return;
    }
...
SQLiteInnerAccessor.prototype.readyFields = function (callback) {
    ...
    if (fields != null && fields.length > 0) { return callback(); }
...
SQLiteInnerAccessor.prototype.insert = function (dataObject, callback) {
    var self = this;
    var idAttribute = this.autoIncrement?this.idAttribute :null;
    var prepare = this.sqlBuilder.buildInsertSQL(dataObject, self.tablename, self.fields, idAttribute);
    var parameters = prepare.parameters;
    this.execute(prepare.sql, prepare.parameters, function (err, info) {
        if (err == null && self.autoIncrement) {
            self.query(framework.String.Format("select {0} from [{1}] order by {0} desc limit 1;", idAttribute, self.tablename), [], function (err2, info) {
                self.dispatch("INSERT");
                self.Env.Call(callback, err2, info[0][idAttribute]);
            });
        } else {
            self.dispatch("INSERT");
...
SQLiteInnerAccessor.prototype.update = function (dataObject, options, callback) {
    var self = this;
    var idAttribute = this.autoIncrement?this.idAttribute :null;
    var context = self.sqlBuilder.buildSQLUpdateSQL(dataObject, self.tablename, self.fields, options, idAttribute);
    self.execute(context.sql, context.parameters, function (err, info) {
...
SQLiteInnerAccessor.prototype.delete = function (options, callback) {
    var self = this;
    var context = self.sqlBuilder.buildDeleteSQL(self.tablename, options);
    self.execute(context.sql, context.parameters, function (err, info) {
...
SQLiteInnerAccessor.prototype.select = function () {
    ...
    var context = self.sqlBuilder.buildSelectSQL(self.tablename, options);
    self.query(context.sql, context.parameters, function (err, dataset) {
...
SQLiteInnerAccessor.prototype.selectEach = function (options, callback, complete) {
    var self = this;
    var context = self.sqlBuilder.buildSelectSQL(self.tablename, options);
    self.queryEach(context.sql, context.parameters, function (err, row) {
...
//带有事务监听的指定sql语句
SQLiteInnerAccessor.prototype.execute = function (sql, parameters, callback) {
    var self = this;
    var db = self.db;
    db.parallelize(function () {
        db.run(sql, parameters, function (err, data) {
...
//指定指定sql语句查询
SQLiteInnerAccessor.prototype.query = function (sql, parameters, callback) {
    if (!this.tryConnect()) {
        //没有数据库连接,请检查是否配置连接路径
        return callback(framework.InfoMessageStrings.sqliteNoConnectionString);
    }
    if (framework.CheckQuestion.isFunction(parameters)) {
        callback = parameters;
        parameters = [];
    }
    var self = this;
    ...
    self.db.parallelize(function () {
        self.db.all(sql, parameters, function (err, data) {
            if (err) {
                self.log(err);
...
SQLiteInnerAccessor.prototype.queryEach = function (sql, parameters, itemHandle, complete) {
    if (!this.tryConnect()) {
        //没有数据库连接,请检查是否配置连接路径
        return callback(framework.InfoMessageStrings.sqliteNoConnectionString);
    }
    ...
    var proc = process;
    self.db.parallelize(function () {
        self.db.each(sql, parameters, function (err, row) {
            if (err) {
                self.log(err + '\r\n :' + sql);
...
//切换表
SQLiteAccessor.prototype.changeTable = function (newtable, id, autoIncrement) {
    return innerAccessor.changeTable(newtable, id, autoIncrement);
}
//直接执行(exec)sql语句
...
//直接执行select语句
SQLiteAccessor.prototype.query = function (sql, callback) {
    return innerAccessor.query(sql, [], callback);
}
